"""
Program: main.py
Created by Richelle Louise Tobias/Kevin Rayne B. Samonte/Vien Rissy Santos (GROUP 6)
...
def main():
    ...
    #Declaring the Cash Box Class from CashBoxClass.py to a variable
    cash_box = CashBoxClass.CashBox()
    ...
    while True: #Used for looping when the customer wants to order again
        while True:
            try:
        ...
        if inventory.isEmpty(ice_cream.quantity): #Checks if the quantity of the chosen ice cream is empty 
            continue
        
        #PS decrements the bought ice cream, needs to be renovated to update the original quantity in the inventory list
        ...
        #Computes and displays total cash inserted inside the cash box
        print("\nTotal Cash: Php ", cash_box.get_total_cash(vending_machine.money_inserted))

        #Computes and displays total change left inside the cash box
        print("Total Change: Php ", cash_box.get_total_change(vending_machine.money_inserted, ice_cream.price))
        ...
        vending_machine.money_inserted = 0.00

        ctr = input("Do you want to have another order? Please enter 1 if YES and 2 if NO: ")
        if ctr == "1":
            vending_machine.display_ice_cream() 
            continue
        else:
